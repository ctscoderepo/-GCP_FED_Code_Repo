...
const ConfirmationComponent = props => {
  ...
  console.log("from confirmation", storeProductInfo);
  console.log("from confirmation shipToStoreData", shipToStoreData);
  ...
  const userAddress =
    userData && userData.userDetails ? userData.userDetails.address : "";
  const totalOrderValue = checkoutData
    ? checkoutData.totalPrice +
      checkoutData.totalShipping +
      checkoutData.totalTax
    : 0;

  const shippingStoreDetails = storeProductInfo ? (
    <div className={classes.root}>
      <div className={classes.confirmContainer}>
        ...
        <div className={classes.confirmHeader}>
          Your product is shipped to store successfully
        </div>
          ...
          <Grid container>
            <Grid item lg={12} sm={12} xs={12}>
              <div>
                ...
                <span>
                  {storeProductInfo && storeProductInfo.product
                    ? storeProductInfo.product.productName
                    : ""}
                </span>
              </div>
            </Grid>
            <Grid item lg={12} sm={12} xs={12} style={{ marginTop: "10px" }}>
              <div>
                <Typography>
                  ...
                  <strong>
                    {storeProductInfo && storeProductInfo.storeName
                      ? storeProductInfo.storeName
                      : ""}
                  </strong>
                </Typography>
              </div>
        ...
        <Grid container>
          ...
          <Grid item lg={6} sm={6} xs={12} style={{ marginTop: "15px" }}>
            <div className={classes.needHelp}>
              <Typography>
                <strong>Need Help or Questions?</strong>{" "}
              </Typography>
              <Typography>
                <strong>Call :</strong> 1-800-000-0000
              </Typography>
            </div>
          </Grid>
        </Grid>
  ...
  return (
    <div>
      ...
      ) : storeProductInfo && shipToStoreData ? (
        <div>{shippingStoreDetails}</div>
      ) : checkOutLength ? (
        <div className={classes.root}>
          <div className={classes.confirmContainer}>
            <div>
              ...
              <Grid container>
                <Grid item lg={12} sm={12} xs={12}>
                  <div>
                    ...
                    <span>{checkoutData ? checkoutData.orderId : ""}</span>
                  </div>
                </Grid>
                <Grid
                  item
                ...
                >
                  <div>
                    <Typography>
                      Your order of{" "}
                      <strong>${totalOrderValue ? totalOrderValue : 0}</strong>{" "}
                      has been confirmed. You will receive an email cofirmation
                      at{" "}
                      <strong>{orderDetails ? orderDetails.email : ""}</strong>
                    </Typography>
                  </div>
                </Grid>
              </Grid>
